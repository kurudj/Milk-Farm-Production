...
/*
 * class that creates a new scene when a button 3 is pressed in the main window
 */
public class Button3Press {

	Pane root = new Pane();
	static File writeFile; // file where data is printed
	static boolean b = false; // used further to check if button 3 is pressed
	static Farms farms;
	static double[] arr; // button 3 displaying method differs from other buttons
							// so i used an array to store monthly data

	/*
	 * setting up the scene for button 3 pressed
	 */
	public Button3Press(Stage stage) {
		...
		Label lbl2 = new Label("Input farm ID:");
		lbl2.setLayoutX(625);
		lbl2.setLayoutY(170);
		...
		sbmt.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				flRdr.readFile();
				int yearNumber = Integer.parseInt(input.getText());
				int farmId = Integer.parseInt(input2.getText());
				Reports r = new Reports(yearNumber, farmId, flRdr.readFile()); // new report
				arr = r.farmReport(); // get data from report
				writeFile = new File("./Farm report.txt");
